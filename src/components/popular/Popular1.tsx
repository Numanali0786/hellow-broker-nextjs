"use client";
import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from "../ui/carousel";
import { useUser } from "@clerk/nextjs";
import axios from "axios";
import Image from "next/image";
import { PropertyList } from "@/utils/types";
import useSWR from "swr";
const Popular = () => {
  const { isLoaded, user } = useUser();
  const isFetch = isLoaded && user;

  const fetcher = (url: string) => axios.get(url).then((res) => res.data);
  const {
    data: lists,
    isLoading,
    error,
  } = useSWR<PropertyList>(
    isFetch ? `/api/properties/filtered?type=popular` : null,
    fetcher
  );
  if (isLoading) return <p>ffd</p>;
  if (error || !lists || lists.length < 1) return null;

  return (
    <section className="bg-button-pink py-20 px-10">
      <p className="text-center text-[24px] font-bold mx-auto text-white">
        Discover Popular Properties
      </p>
      <p className="text-center text-[14px] text-white mb-8">
        The most searched projects in Delhi South West
      </p>
      <div className="">
        <Carousel className="">
          <CarouselContent className="flex  gap-8">
            {lists.map((item, i) => (
              <CarouselItem
                key={i}
                className="ml-4 w-full bg-white md:basis-1/2 lg:basis-1/4"
              >
                {
                  item.media.length>0?(
                    <Image
                  src={item.media[0].url}
                  alt=""
                  width={"100"}
                  height={"100"}
                  className="w-full object-contain"
                />
                  ):(
                     <Image src={'data:image/jpeg;base64,'} alt="img" className="w-full rounded-lg mt-6 pb-4"/>
                  )
                }
                <div className="p-4 text-primary-subheading">
                  <p className="text-[18px] text-primary-light font-bold">
                    {item.title}
                  </p>
                  <p className="text-[13px] ">{item.title}</p>
                  <p className="text-[14px] ">{item.description}</p>
                  <p className="text-[14px] ">{item.title}</p>
                  <p className="text-[16px] text-primary-light">{item.title}</p>
                </div>
              </CarouselItem>
            ))}
          </CarouselContent>
          <CarouselPrevious />
          <CarouselNext />
        </Carousel>
      </div>
    </section>
  );
};

export default Popular;
